...
namespace prmToolkit.NotificationPattern
{
    public class ValidationContract<T> where T : Notifiable
    {
        ...
        /// <summary>
        /// Given a string, add a notification for null or void and its length to less than the parameter less than the maximum parameter
        /// </summary>
        /// <param name="selector">Property</param>
        /// <param name="message">Error Message (Optional)</param>
        /// <returns></returns>
        public ValidationContract<T> IsRequiredWithSize(Expression<Func<T, string>> selector, int min, int max, string message = "")
        {
            var val = selector.Compile().Invoke(_validatable);
            var name = ((MemberExpression)selector.Body).Member.Name;
            ...
            if (string.IsNullOrEmpty(val) ||
               (!string.IsNullOrEmpty(val) && val.Length < min) ||
               (!string.IsNullOrEmpty(val) && val.Length > max))
